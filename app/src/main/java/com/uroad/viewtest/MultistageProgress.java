...
public class MultistageProgress extends View {
    ...
    public  static int DEF_COLORS[];//默认背景颜色数组
    public  static float DEF_WEIGHTS[];//每段对应的权重
...
//        init();
    ...
    public MultistageProgress(Context context,int[] colorss,float[] weightss) {
        super(context);
        this.DEF_COLORS = colorss;
        this.DEF_WEIGHTS = weightss;
        init();
    }
...
//            int width = (int) (getWidthForWeight(weights[i], totalWeight)); //绘制矩形块之间的分割线
//            lineX = lineX + width;
...
//        int progressX = (int) getWidthForWeight(progress, maxProgress);
    ...
    public float getWidthForWeight(float weight, float totalWeight) {
        return getHeight() * (weight / totalWeight) + 0.5f;
    }
